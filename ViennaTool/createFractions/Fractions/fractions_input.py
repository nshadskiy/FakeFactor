...
class Era:
    ...
    def __init__(self, era, channel, shift = "NOMINAL"):
        ...
        if era == "2017_KIT_emb": # It is important that the "emb" is part of the era name if you want to use embedded samples
            # Cut.cutfile = "/home/jandrej/CMSSW_9_4_0/src/HephyHiggs/Tools/Datacard/conf2016/cuts.json"
            self.lumi = 41.53
            self.weights = "weight"
            self.tauidsfs = {
            ...
            self.add_weights = self.weights.replace(" ","").split("*") 
            path = "/ceph/jbechtel/2017_v14/{0}-".format( channel)
            ext = ".root"
