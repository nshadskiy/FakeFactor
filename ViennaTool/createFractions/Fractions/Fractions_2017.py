...
class Fractions():
    ...
    def fillHisto(self, path, cut, name, weight = True):
        ...
        stitch_weight_Wjets = "((0.0010062794683*((npartons <= 0 | npartons >= 5)*1.0 + (npartons == 1)*0.1448 + (npartons == 2)*0.0887 + (npartons == 3)*0.0540 + (npartons == 4)*0.0535)) * (genbosonmass>=0.0) + numberGeneratedEventsWeight * crossSectionPerEventWeight * (genbosonmass<0.0))"
        stitch_weight_DY = "((genbosonmass >= 50.0)*6.3654e-05*((npartons == 0 | npartons >= 5)*1.0 + (npartons == 1)*0.1743 + (npartons == 2)*0.3556 + (npartons == 3)*0.2273 + (npartons == 4)*0.2104) + (genbosonmass < 50.0)*numberGeneratedEventsWeight*crossSectionPerEventWeight)"
        ...
        if "Data" not in path :
            if "EMB" not in path :
                ...
                trg_weight = "1*"
                if "tt-" in path :
                    trg_weight = "(0.18321*(pt_1>=30 & pt_1<35) + 0.53906*(pt_1>=35 & pt_1<40) + 0.63658*(pt_1>=40 & pt_1<45) + 0.73152*(pt_1>=45 & pt_1<50) + 0.8*(pt_1>=50))"# & pt_1<60) + 0.84666*(pt_1>=60 & pt_1<80) + 0.84919*(pt_1>=80 & pt_1<100) + 0.86819*(pt_1>=100 & pt_1<150) + 0.88206*(pt_1>=150 & pt_1<200) + (pt_1>=200))"
                    trg_weight += "*(0.18321*(pt_2>=30 & pt_2<35) + 0.53906*(pt_2>=35 & pt_2<40) + 0.63658*(pt_2>=40 & pt_2<45) + 0.73152*(pt_2>=45 & pt_2<50) + 0.8*(pt_2>=50))*"#" & pt_2<60) + 0.84666*(pt_2>=60 & pt_2<80) + 0.84919*(pt_2>=80 & pt_2<100) + 0.86819*(pt_2>=100 & pt_2<150) + 0.88206*(pt_2>=150 & pt_2<200) + (pt_2>=200))*"
                weights = trg_weight
        ...
        if add_weights :
            col = ["puweight","eleTauFakeRateWeight","muTauFakeRateWeight","idWeight_1","isoWeight_1","trackWeight_1",
                                    ...
                                    "crossSectionPerEventWeight"]+add_weights+self.var+ ["gen_match_1","gen_match_2","decayMode_1","decayMode_2","pt_2","pt_1"] 
        else:
            col = ["puweight","eleTauFakeRateWeight","muTauFakeRateWeight","idWeight_1","isoWeight_1","trackWeight_1",
                                    "singleTriggerDataEfficiencyWeightKIT_1","singleTriggerMCEfficiencyWeightKIT_1","numberGeneratedEventsWeight",
                                    "crossSectionPerEventWeight"]+self.var+ ["gen_match_1","gen_match_2","decayMode_1","decayMode_2","pt_2","pt_1"] 
        ...
        tmp =   rp.read_root( paths = path,
                              ...
                              columns = col,
                              key="{}_nominal/ntuple".format(self.channel)
                            )
