...
void FFCalculator::calc_nonclosure(const Int_t mode, const TString raw_ff, const TString compare_file, TString ff_output, const TString tight_cat, const Int_t doPlot, const Int_t subtractMC, const Int_t tau_ind){
  ...
  Double_t fitWidth;
  if(mode & _QCD) fitWidth=1.2; else if(mode & _W_JETS) fitWidth=1.2; else fitWidth=1.2;
  if(CHAN==kTAU) fitWidth=1.2;
...
void FFCalculator::calc_nonclosure_lepPt(const Int_t mode, const TString raw_ff, const TString compare_file, TString ff_output, const TString tight_cat, const Int_t doPlot, const Int_t subtractMC, const Int_t tau_ind){
  ...
  if(subtractMC){
    ...
    for (Int_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (mode & SR){
        ...
        if (  this->isInSR(mode,tau_ind) && this->isLoose(mode,tau_ind) ){
          if( !raw_ff.Contains("_fitted") ) 
            { FF_value = FF_lookup_h->GetBinContent( this->getBin(mode|tau_ind)+1 );
<<<<<<< Updated upstream
=======
<<<<<<< Updated upstream
=======
              cout << "FF value1: " << FF_value << endl;
>>>>>>> Stashed changes
>>>>>>> Stashed changes
            }
          else if( raw_ff.Contains("_fitted") ){
            FF_value = this->getFittedBinContent( mode, fittedFFs );
            cout << "FF value2: " << FF_value << endl;
          }
          cout << FF_value << event_s->weight_sf << nonclosure_h->GetBinContent( this->getWeightIndex_mvis(event_s->alltau_mvis->at(tau_ind) )+1 ) << endl;
        ...
        cout << "CR mode" << endl;
        cout << "isInSR: " << this->isInSR(mode,tau_ind) << endl;
        cout << "isInCR: " << this->isInCR(mode,tau_ind) << endl;
        cout << "isLoose: " << this->isLoose(mode,tau_ind) << endl;
