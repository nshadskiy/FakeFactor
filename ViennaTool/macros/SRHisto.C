...
void SRHisto(int doCalc, int DOCUTS) {
  ...
  for(Int_t icat=0; icat<nCAT; icat++){
    ...
    const TString r2[nVARused]={ "_mt", "_mvis", "_pt"};
    ...
    for (unsigned i=0; i<ps.size(); i++){
      ...
      //if (DOCUTS) for (unsigned l=0; l<NC; l++){ tmp2=tmp; Analyzer->calcBgEstSim( ps.at(i), MT|NO_SR, categoryMode, tmp2.ReplaceAll("SR_","SR_cuts_"+c_text[l]+"_"), c_cuts[l]  ); }       
      ...
      //if (DOCUTS) for (unsigned l=0; l<NC; l++){ tmp2=tmp; Analyzer->calcBgEstSim( ps.at(i), MVIS, categoryMode, tmp2.ReplaceAll("SR_","SR_cuts_"+c_text[l]+"_"), c_cuts[l]  ); }
    ...
    //  const int NC=5;
    TH1D* hsum[nVAR][nSR];
    TH1D* hsum_cuts[nVAR][nSR][NC];
    TH1D* hsum_woQCD[nVAR][nSR];
    TH1D* hsum_cuts_woQCD[nVAR][nSR][NC];
    TH1D* hsum_SS[nVAR][nSR];
    ...
    TH1D* h_mt70_mc[nVAR][nSR];
    for (unsigned kk=0; kk<nVAR; kk++){
    ...
    //QCD OS/SS
    cout << "QCD OS/SS" << endl;
    TH1D* hqcd_OS_SS_MCsum[nVAR][nSR];
    TH1D* hqcd_OS_SS_data[nVAR][nSR];
    TH1D* hqcd_OS_SS_factor[nVAR];
    for (unsigned kk=0; kk<nVAR; kk++){
...
int main(int argc, char* argv[]) {
  ...
  int doCuts = argc > 2 ? atoi(argv[2]) : 1;
  SRHisto(doCalc,doCuts);    
}
#endif
